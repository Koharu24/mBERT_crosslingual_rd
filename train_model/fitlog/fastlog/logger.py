...
def _check_log_dir(func):
    ...
    def wrapper(*args, **kwargs):
        if not args[0].initialized and args[0].default_log_dir is not None:
            ...
            raise RuntimeError(
                "You have to call `fitlog.set_log_dir()` to set where to save log first."
            )
        return func(*args, **kwargs)
...
class Logger:
    ...
    def commit(self, file: str = None, fit_msg: str = None):
        ...
        if msg["status"] == 0:  # 成功了
            ...
            print(
                _colored_string(
                    "Put your code inside a fitlog project, or use 'fitlog init' to initialize it\n",
                    "red",
                )
            )
            raise RuntimeError(
    ...
    def set_log_dir(self, log_dir: str, new_log: bool = False):
        ...
        if not os.access(log_dir, os.W_OK):
            raise PermissionError(
                "write is not allowed in `{}`. Check your permission.".format(log_dir)
            )
        ...
        if not hasattr(self, "meta_logger"):
            ...
            for name in [
                ...
                handler = logging.FileHandler(
                    os.path.join(self._save_log_dir, "{}.log".format(name)),
                    encoding="utf-8",
                )
    ...
    def __add_meta(self):
        ...
        for value, name in zip(
            [self.fit_id, self.git_id, self.fit_msg, self.git_msg],
    ...
    def finish(self, status: int = 0, send_to_bot: str = None):
        ...
        if hasattr(self, "meta_logger"):
            ...
            self._write_to_logger(json.dumps(_dict), "meta_logger")
        self.add_other(
            value=get_hour_min_second(time.time() - self._start_time), name="cost_time"
        )
    ...
    def add_to_file(self, value: Union[str, dict]):
        ...
        assert isinstance(
            value, (str, dict)
        ), "Only str or dict allowed, not {}.".format(type(value))
    ...
    def add_metric(
        ...
        ), "Only positive integer is allowed to be `step`."
        _dict = _parse_value(value, name, parent_name="metric")
        _dict["step"] = step
        if epoch is not None:
            assert (
                isinstance(epoch, int) and epoch > -1
            ), "Only positive integer is allowed to be `epoch`."
            _dict["epoch"] = epoch
    ...
    def add_loss(
        ...
        self._write_to_logger(
            _str, "loss_logger"
        )  # {'loss': {}, 'step':xx, 'epoch':xx}

    @_check_debug
    @_check_log_dir
    def add_hyper(
        self,
        value: Union[int, str, float, dict, argparse.Namespace, ConfigParser],
        name=None,
    ):
        """
    ...
    def add_other(self, value: Union[int, str, float, dict], name: str = None):
        ...
        if name in ("meta", "hyper", "metric", "loss") and not isinstance(value, dict):
            raise KeyError(
                "Don't use {} as a name. Use fitlog.add_{}() to save it.".format(
                    name, name
                )
            )
            ...
            for line in lines:
                ...
                if len(re.findall("^#####+hyper$", line)) != 0:
                    ...
                    if len(line) != 0 and not line.startswith("#"):
                        ...
                        # 删除str开头结尾的'"
                        last_value = (
                            values[-1].rstrip("'").rstrip('"').lstrip("'").lstrip('"')
                        )
                        if last_value == "False":
                            last_value = False
    ...
    def add_progress(self, total_steps: int = None):
        ...
        self._write_to_logger(
            json.dumps({"total_steps": total_steps}), "progress_logger"
        )
...
def _parse_value(
    value: Union[int, str, float, dict], name: str, parent_name: str = None
) -> dict:
    """
    ...
    """
    if name is not None:
        assert isinstance(name, str), "name can only be `str` type, not {}.".format(
            name
        )
    _dict = {}
    ...
    if isinstance(value, (int, float, str, bool)) or value is None:
        if name is None:
            raise RuntimeError(
                "When value is {}, you must pass `name`.".format(type(value))
            )

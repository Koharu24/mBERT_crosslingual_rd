...
class MixUnalignBertPipe(Pipe):
    ...
    def process(self, data_bundle):
        ...
        data_bundle.apply_field(
            lambda x: x.replace("<concept>", "[MASK]"),
            field_name="definition",
            new_field_name="definition",
        )
        ...
        for lg in lgs:
            ...
            names = [
                ...
                if name.startswith(lg) and "test" not in name
            ]
        ...
        for name in data_bundle.get_dataset_names():
            ...
            for ins in ds:
                ...
                for word in definition:
                    ...
                    words.extend(word)
                input = [cls_id] + [mask_id] * self.max_word_len + [sep_id] + words
                input = input[:256]
                input.append(sep_id)
                ins["input"] = input
        ...
        for word2bpes in [train_word2bpes, target_word2bpes]:
            for i in range(len(word2bpes)):
                bpes = word2bpes[i]
                bpes = bpes[: self.max_word_len] + [mask_id] * max(
                    0, self.max_word_len - len(bpes)
                )
                word2bpes[i] = bpes
